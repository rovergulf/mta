...
func Example_daemon() {
	ch := make(chan *mta.Message)

	go func() {
		d := mta.NewDialer("smtp.example.com", 587, "user", "123456")

		var s mta.SendCloser
		var err error
		open := false
		for {
			select {
...
func Example_noAuth() {
	...
	d := mta.Dialer{Host: "localhost", Port: 587}
	if err := d.DialAndSend(m); err != nil {
		panic(err)
	}
}
...
func Example_noSMTP() {
	...
	s := mta.SendFunc(func(from string, to []string, msg io.WriterTo) error {
...
func ExampleSetCopyFunc() {
	m.Attach("foo.txt", mta.SetCopyFunc(func(w io.Writer) error {
...
func ExampleSetHeader() {
	...
	m.Attach("foo.jpg", mta.SetHeader(h))
}

func ExampleRename() {
	m.Attach("/tmp/0000146.jpg", mta.Rename("picture.jpg"))
}
...
func ExampleSetCharset() {
	m = mta.NewMessage(mta.SetCharset("ISO-8859-1"))
}

func ExampleSetEncoding() {
	m = mta.NewMessage(mta.SetEncoding(mta.Base64))
}

func ExampleSetPartEncoding() {
	m.SetBody("text/plain", "Hello!", mta.SetPartEncoding(mta.Unencoded))
}
